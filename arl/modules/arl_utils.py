    ...
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                label_size = len(open("data/knowledge/entity2id.txt").read().strip().split("\n")[1:])
                setattr(pl_module, f"{split}_{k}_img_aucroc", AUROC(label_size))
                setattr(pl_module, f"{split}_{k}_img_f1", F1(label_size))
                setattr(pl_module, f"{split}_{k}_txt_aucroc", AUROC(label_size))
                setattr(pl_module, f"{split}_{k}_txt_f1", F1(label_size))
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            pl_module.log(f"{loss_name}/{phase}/score_best_epoch",
                          getattr(pl_module, f"{phase}_{loss_name}_score").get_best_score(), sync_dist=True)
            pl_module.log(f"{loss_name}/{phase}/close_score_best_epoch",
                          getattr(pl_module, f"{phase}_{loss_name}_score").get_best_close_score(), sync_dist=True)
            pl_module.log(f"{loss_name}/{phase}/open_score_best_epoch",
                          getattr(pl_module, f"{phase}_{loss_name}_score").get_best_open_score(), sync_dist=True)
            getattr(pl_module, f"{phase}_{loss_name}_score").reset()

            pl_module.log(f"{loss_name}/{phase}/loss_epoch", getattr(pl_module, f"{phase}_{loss_name}_loss").compute(), sync_dist=True)
            ...
            pl_module.log(f"{loss_name}/{phase}/irtr_loss_epoch", value, sync_dist=True)
            ...
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value, sync_dist=True)
            pl_module.log(f"{loss_name}/{phase}/loss_epoch", - value, sync_dist=True)
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()
        ...
        elif loss_name == "align":
            if phase != "train":
                pl_module.log(f"{loss_name}/{phase}/img_f1_epoch",
                              getattr(pl_module, f"{phase}_{loss_name}_img_f1").compute(), sync_dist=True)
                getattr(pl_module, f"{phase}_{loss_name}_img_f1").reset()

                pl_module.log(f"{loss_name}/{phase}/txt_f1_epoch",
                              getattr(pl_module, f"{phase}_{loss_name}_txt_f1").compute(), sync_dist=True)
                getattr(pl_module, f"{phase}_{loss_name}_txt_f1").reset()

                pl_module.log(f"{loss_name}/{phase}/img_aucroc_epoch",
                              getattr(pl_module, f"{phase}_{loss_name}_img_aucroc").compute(), sync_dist=True)
                getattr(pl_module, f"{phase}_{loss_name}_img_aucroc").reset()

                pl_module.log(f"{loss_name}/{phase}/txt_aucroc_epoch",
                              getattr(pl_module, f"{phase}_{loss_name}_txt_aucroc").compute(), sync_dist=True)
                getattr(pl_module, f"{phase}_{loss_name}_txt_aucroc").reset()
...
def set_schedule(pl_module):
    ...
    if "pretrain" in pl_module.hparams.config["exp_name"]:
        head_names += ["align_head"]
