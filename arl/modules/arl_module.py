...
class ARLTransformerSS(nn.Module):
    def __init__(self, config, outputpath=None):
        ...
        if config["loss_names"]["align"] > 0 or "finetune" in self.hparams.config["exp_name"]:
            self.align_head = prediction_heads.AlignHead(config)
            self.align_head.apply(init_weights)
            if "finetune" in self.hparams.config["exp_name"]:
                for param in self.align_head.parameters():
                    param.requires_grad = self.hparams.config["finetune_embeddings"]
    ...
    def infer(
        ...
        # == Begin: Return Uni-modal Feats ==
        if unimodal:
            ret["uni_modal_text_feats"] = uni_modal_text_feats[:, 0]
            ret["uni_modal_image_feats"] = uni_modal_image_feats[:, 0]
            return ret
        # == End  : Return Uni-modal Feats ==

        # == Begin: Reason Using Knowledge ==
        if mask_text:
        ...
        ret["attentions"] = {"text2image": [], "image2text": [], "ent2image": []} if output_attentions else None
        ...
        for layer_idx, (text_layer, image_layer) in enumerate(zip(self.multi_modal_language_layers,
            ...
            x1, z1 = text_layer(x, y, extended_text_masks, extended_image_masks, output_attentions=True,
                                knowledge_hidden_states=z, knowledge_attention_mask=extend_ent_masks,
                                knowledge_position_matrix=ent_pos_matrices)
            ...
            if output_attentions:
                ret["attentions"]["text2image"].append(x1[1:])
                ret["attentions"]["image2text"].append(y1[1:])
                ret["attentions"]["ent2image"].append(z1[1:])
    ...
    def forward(self, batch, test=False):
        ...
        # Pre-Training: Aligning Through Knowledge
        if "align" in self.current_tasks:
            ret.update(objectives.compute_align(self, batch))
